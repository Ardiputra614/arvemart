...
export default function Navbar({ user: serverUser }) {
  ...
  // =========================
  // 🔥 FETCH USER (FIX UTAMA)
  // =========================
  const fetchUser = async () => {
    try {
      const res = await axios.get("https://api.arvemart.com/api/me", {
      ...
      <nav className="bg-[#1a191d] border-b border-gray-800 sticky top-0 z-50">
          ...
          <div className="flex items-center justify-between h-16 gap-4">
            ...
            <Link href="/" className="flex items-center gap-2">
              {profil?.logo ? (
                <img
                  src={profil.logo}
                  alt={profil.application_name || "ARVE"}
                  className="h-8 w-auto object-contain"
                />
              ) : (
                <span className="text-white font-bold text-lg">
                  {profil?.application_name || "ARVE"}
                </span>
              )}
            </Link>
        ...
        {open && (
          <div className="md:hidden bg-[#1a191d] border-t border-gray-800 px-4 py-4 space-y-4">
            <Link href="/" onClick={() => setOpen(false)} className="block text-gray-300">
              Topup
            </Link>

            <Link href="/cek-transaksi" onClick={() => setOpen(false)} className="block text-gray-300">
              Cek Transaksi
            </Link>

            {user ? (
              <>
                <Link href="/history" onClick={() => setOpen(false)} className="block text-gray-300">
                  History
                </Link>

                <Link href="/profile" onClick={() => setOpen(false)} className="block text-gray-300">
                  Profile
                </Link>
              ...
              <Link href="/login" onClick={() => setOpen(false)} className="block text-gray-300">
                Login
              </Link>
            )}
          </div>
      ...
      {mobileSearchOpen && (
        <div className="fixed inset-0 bg-[#1a191d] z-[999] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingSearch && <div className="p-4 text-gray-400">Loading...</div>}
            {!loadingSearch && query && results.length === 0 && (
