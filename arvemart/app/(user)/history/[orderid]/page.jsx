...
export default function History() {
  ...
  const getInstructions = (type, methodName) => {
    const instructions = {
      bank_transfer: [
        `Buka aplikasi ${methodName?.toUpperCase() || "bank"} Anda`,
        "Pilih menu Transfer / Bayar",
        "Pilih Virtual Account / Rekening",
      ...
      cstore: [
        ...
        `Datang ke ${methodName || "toko"} terdekat`,
        "Tunjukkan kode pembayaran ke kasir",
        "Bayar sesuai nominal yang tertera",
      ],
      ewallet: [
        "Klik tombol \"Bayar Sekarang\" di bawah",
        `Anda akan diarahkan ke aplikasi ${methodName || "e-wallet"}`,
        "Konfirmasi pembayaran di aplikasi tersebut",
      ],
    };
  ...
  const downloadInvoice = async () => {
    try {
      ...
      if (finalData.ipaymu_reference) {
        fieldRow("ID Referensi", finalData.ipaymu_reference, true);
      }
      y += 4;
      ...
      const voucherDiscount = parseFloat(finalData.voucher_discount || 0);
      const appFee = parseFloat(finalData.application_fee || 0);
      const fee = grossAmount - sellingPrice + voucherDiscount - appFee;

      fieldRow("Harga Produk", formatRupiah(sellingPrice));
      if (fee > 0) fieldRow("Biaya", formatRupiah(fee));
      if (appFee > 0) fieldRow("Biaya Layanan", formatRupiah(appFee));
      if (voucherDiscount > 0) fieldRow("Diskon Voucher", "-" + formatRupiah(voucherDiscount));
  ...
  const handlePay = () => {
    ...
    window.open(finalData.ipaymu_payment_url, "_blank");
  };
  ...
  // Helper untuk mendapatkan payment URL dari iPaymu response
  const getPaymentUrl = useCallback((data) => {
    if (data.ipaymu_payment_url) return data.ipaymu_payment_url;
    if (data.payment_type === "qris" && data.ipaymu_qr_string)
      return data.ipaymu_qr_string;
    return null;
  }, []);

  const getVaNumber = useCallback((data) => {
    if (data.ipaymu_va) return data.ipaymu_va;
    return null;
  }, []);

  // Helper untuk mendapatkan expiry time dari iPaymu
  const getExpiryTime = useCallback((data) => {
    if (data.ipaymu_expiry) return new Date(data.ipaymu_expiry);
    if (data.ipaymu_response) {
      let ipaymuResp = data.ipaymu_response;
      if (typeof ipaymuResp === "string") {
        try {
          ipaymuResp = JSON.parse(ipaymuResp);
        } catch (e) {
  ...
  const displayVaNumber =
    finalData.ipaymu_va || finalData.va_number || finalData.url;
    ...
    <>
      ...
      <div className="min-h-screen text-white py-8">
        <div className="max-w-4xl mx-auto px-4">
            ...
            displayVaNumber && (
              <div className="bg-gray-800 rounded-xl p-6 mb-6">
                ...
                <button
                  ...
                  className="flex items-center justify-between w-full mt-4 px-4 py-3 bg-gray-900/50 rounded-lg text-sm text-gray-300 hover:text-white transition"
                >
                  <span>Tata Cara Pembayaran</span>
                  <ChevronDown
                    size={16}
                    className={`transition-transform ${
                      openAccordion === "va" ? "rotate-180" : ""
                    }`}
                  />
                </button>
                {openAccordion === "va" && (
                  <ol className="mt-3 text-sm text-gray-400 space-y-2 pl-5 list-decimal">
                    {getInstructions("bank_transfer", finalData.payment_method_name).map((step, i) => (
                      <li key={i}>{step}</li>
                    ))}
                  </ol>
                )}
            ...
            <div className="bg-gray-800 rounded-xl p-6 mb-6">
              ...
              <div className="flex flex-col items-center gap-4">
                {finalData.ipaymu_qr_string && (
                  <>
                    ...
                    >
                      <QRCodeCanvas
                        value={finalData.ipaymu_qr_string}
                        size={220}
                        level="H"
                        includeMargin={true}
                      />
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleCopy(finalData.ipaymu_qr_string)}
              ...
              >
                ...
                <ChevronDown
                  size={16}
                  className={`transition-transform ${
                    openAccordion === "qris" ? "rotate-180" : ""
                  }`}
                />
              </button>
              {openAccordion === "qris" && (
                <ol className="mt-3 text-sm text-gray-400 space-y-2 pl-5 list-decimal">
                  {getInstructions("qris", finalData.payment_method_name).map((step, i) => (
                    <li key={i}>{step}</li>
                  ))}
                </ol>
              )}
            ...
            <div className="bg-gray-800 rounded-xl p-6 mb-6">
              ...
              <div className="flex justify-between items-center bg-gray-900 p-4 rounded-lg">
                <span className="font-mono text-xl">
                  {finalData.ipaymu_va || finalData.ipaymu_reference}
                </span>
              ...
              {/* Accordion Instructions */}
              <button
                onClick={() => toggleAccordion("cstore")}
                className="flex items-center justify-between w-full mt-4 px-4 py-3 bg-gray-900/50 rounded-lg text-sm text-gray-300 hover:text-white transition"
              >
                <span>Tata Cara Pembayaran</span>
                <ChevronDown
                  size={16}
                  className={`transition-transform ${
                    openAccordion === "cstore" ? "rotate-180" : ""
                  }`}
                />
              </button>
              {openAccordion === "cstore" && (
                <ol className="mt-3 text-sm text-gray-400 space-y-2 pl-5 list-decimal">
                  {getInstructions("cstore", finalData.payment_method_name).map((step, i) => (
                    <li key={i}>{step}</li>
                  ))}
                </ol>
              )}
          ...
          {/* Payment Info - Ewallet / Redirect */}
          {status === "pending" &&
            finalData.payment_type !== "qris" &&
            finalData.payment_type !== "bank_transfer" &&
            finalData.payment_type !== "cstore" && (
              <div className="bg-gray-800 rounded-xl p-6 mb-6">
                <h3 className="font-semibold mb-4">
                  Pembayaran {finalData.payment_method_name}
                </h3>
                ...
                >
                  ...
                  <ChevronDown
                    size={16}
                    className={`transition-transform ${
                      openAccordion === "ewallet" ? "rotate-180" : ""
                    }`}
                  />
                </button>
                {openAccordion === "ewallet" && (
                  <ol className="mt-3 text-sm text-gray-400 space-y-2 pl-5 list-decimal">
                    {getInstructions("ewallet", finalData.payment_method_name).map((step, i) => (
                      <li key={i}>{step}</li>
                    ))}
                  </ol>
                )}
            ...
            <div className="space-y-3">
              ...
              <div className="border-t border-gray-700 pt-4 mt-4 space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatRupiah(finalData.selling_price)}</span>
                </div>
                {(() => {
                  const appFee = parseFloat(finalData.application_fee || 0);
                  const fee = parseFloat(finalData.gross_amount || 0) - parseFloat(finalData.selling_price || 0) + parseFloat(finalData.voucher_discount || 0) - appFee;
                  return fee > 0 ? (
                    <div className="flex justify-between">
                ...
                {parseFloat(finalData.application_fee || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-400">Biaya Layanan</span>
                    <span className="font-medium">{formatRupiah(finalData.application_fee)}</span>
                  </div>
                )}
                {parseFloat(finalData.voucher_discount || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-green-400">Diskon Voucher</span>
                    <span className="font-medium text-green-400">
                      -{formatRupiah(finalData.voucher_discount)}
                    </span>
                  </div>
                )}
              </div>
