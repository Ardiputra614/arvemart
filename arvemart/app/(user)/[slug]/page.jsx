...
const GamesTopup = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [productsRes, serviceRes, paymentRes, profilRes] = await Promise.all([
          api.get(`/api/products/${slug}`),
          api.get(`/api/service/${slug}`),
  ...
  const handlePLNInquiry = async () => {
    ...
    try {
      const response = await api.post("/api/inquiry-pln", {
  ...
  const isFormComplete = () => {
    ...
    return accountOk && plnOk && nameOk && paymentOk && waOk && agreed;
  };
  ...
  const calculateTotalFee = () => {
    ...
    switch (paymentMethod.fee_type) {
      ...
      case "percentage":
        if (paymentMethod.percentage_fee > 0) {
          return (price * paymentMethod.percentage_fee) / 100;
        }
        return 0;
      ...
      case "mixed":
        let percentFee = 0;
        if (paymentMethod.percentage_fee > 0) {
          percentFee = (price * paymentMethod.percentage_fee) / 100;
        }
  ...
  const calculateTotalPayment = () => {
    const price = Number(selectedProduct?.selling_price) || 0;
    const total = price + calculateTotalFee() + applicationFee;
  ...
  const handleApplyVoucher = async () => {
    ...
    try {
      const totalAmount = (Number(selectedProduct?.selling_price) || 0) + calculateTotalFee() + applicationFee;
      ...
      if (res.data.valid) {
        ...
        toast.success(`Diskon ${res.data.data.discount_type === "percentage" ? res.data.data.discount_value + "%" : "Rp " + res.data.data.discount_value.toLocaleString()} berhasil diterapkan!`);
      } else {
        setVoucherData(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post("/api/create-transaction", data);
      toast.success("Berhasil membuat transaksi");
      router.push(`/history/${response.data.order_id}`);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-[#37353E] flex items-center justify-center">
        <div className="text-center p-6">
          <div className="text-white text-lg mb-2">Service tidak ditemukan</div>
          {error && (
          ...
          <Link
            href="/"
            className="inline-block mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700"
          >
            Kembali ke Beranda
          </Link>
        </div>
                                      ...
                                      <div className="text-xs text-gray-400 mt-1">
                                        Total Rp{(
                                          ...
                                          (method.fee_type === "flat"
                                            ? Number(method.nominal_fee) || 0
                                            : method.fee_type === "percentage"
                                              ? ((Number(selectedProduct?.selling_price) || 0) * (method.percentage_fee || 0)) / 100
                                              : method.fee_type === "mixed"
                                                ? (Number(method.nominal_fee) || 0) + ((Number(selectedProduct?.selling_price) || 0) * (method.percentage_fee || 0)) / 100
                                                : 0)
                                        ).toLocaleString()}
                                      </div>
                                    </div>
                    ...
                    <div className="p-6">
                      <h3 className="text-xl font-bold text-gray-100 mb-4">
                        Konfirmasi Pembayaran
                          ...
                          {selectedProduct ? (
                            <>
                              <div className="mb-4 p-3 rounded-lg" style={{ backgroundColor: COLORS.secondary }}>
                                <p className="text-xs text-gray-400 mb-2">Kode Voucher</p>
                                <div className="flex gap-2">
                                  <input
                                    type="text"
                                    value={voucherCode}                                    
                                    ...
                                    className="flex-1 px-3 py-2 rounded-lg text-sm bg-gray-700 text-white border border-gray-600 focus:outline-none focus:border-blue-500 uppercase"
                              ...
                              {applicationFee > 0 && (
                                <div className="flex justify-between items-center">
                                  ...
                                  <span
                                    ...
                                    style={{ color: COLORS.error }}
                                  >
                                    <FormatRupiah value={applicationFee} />
                                  </span>
                                </div>
                              )}
                          ...
                          <label className="flex items-start gap-2 mt-4 mb-3 cursor-pointer">
                            ...
                            <span className="text-sm text-gray-400">
                              Saya menyetujui{" "}
                              <Link href="/term-condition" className="text-blue-400 hover:underline">
