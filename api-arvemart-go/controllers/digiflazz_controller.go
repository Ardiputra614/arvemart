...
// =========================================================
// CATATAN MARGIN (referensi pasaran, TIDAK dipakai saat ini)
// =========================================================
// Kategori            | Margin pasaran umum
// --------------------|---------------------
// Game voucher        | 3% - 5%
// Pulsa               | 1% - 2%
// Paket data/internet | 2% - 3%
// Token/tagihan PLN   | 3% - 5%
// E-money             | 1% - 2%
// Voucher makanan     | 2% - 3%
// Lain-lain           | 3%
//
// Saat ini SEMUA kategori prepaid dipukul rata 5% sesuai permintaan.
// Kalau nanti mau balik ke margin per-kategori, tinggal ganti bagian
// "PREPAID" di bawah dengan logic per-kategori pakai angka di atas.
// =========================================================
...
func calculateMarginByCategory(
	...
	// POSTPAID
	// =========================
	// Perhitungan margin/fee untuk postpaid dinonaktifkan (di-comment)
	// sesuai permintaan. Untuk sementara harga postpaid = harga dasar + admin,
	// tanpa tambahan service fee.
	if productType == "postpaid" {

		// -- DI-COMMENT: logic margin postpaid --
	...
	// PREPAID
	// =========================
	// Margin per kategori sesuai harga pasaran:
	// - Games            : 4%
	// - Pulsa            : 1%  (dikecilkan, dibulatkan ke kelipatan 100)
	// - Data/Internet    : 2%
	// - PLN/Token        : 1%  (dikecilkan, dibulatkan ke kelipatan 100)
	// - E-money          : 2%
	// - Voucher makanan  : 3%
	// - Lain-lain        : 3% (default)
	...
	isSmallMarginCategory := false // khusus PLN & Pulsa: margin kecil + bulat ke ratusan
	...
	if isSmallMarginCategory {
		// PLN & Pulsa: selalu dibulatkan ke atas kelipatan 100
		// supaya tidak melompat jauh dari margin aslinya (mis. 51.980 -> 52.500)
    ...
    log.Printf("MARGIN_CHECK category=%q margin=%.4f small=%v base=%d final=%d", category, marginPercentage, isSmallMarginCategory, price, roundedPrice)
	return roundedPrice
}
...
func GetProducts(c *gin.Context) {
	...
	for _, item := range data {
		...
		// Hitung selling price dengan margin flat 5% (prepaid)
...
func GetProductsPasca(c *gin.Context) {
	...
	for _, item := range data {
		...
		// Untuk postpaid, margin/fee tambahan sudah di-comment di dalam
		// calculateMarginByCategory. Saat ini hasilnya = basePrice + admin.
